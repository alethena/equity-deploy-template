...
contract('Draggable - Unwrap', accounts => {
  let Draggable;
  let EQUITYInstance;
  let XCHFInstance;
  ...
  it("NEG - Can't unwrap if contract is active", async () => {
    await shouldRevert(
      Draggable.unwrap(new BN(100), { from: Tokenholder1 }),
      'As long as the contract is active, you are bound to it'
    );
  });
});
